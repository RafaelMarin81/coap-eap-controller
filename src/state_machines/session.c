...
// ya que se estaría reinicializando y es inútil.
    ...
    //Init the EAP user
	printf("Init EAP peer state machine");
...
void updateSession(char *message, pana_ctx *pana_session) {
	...
	#ifdef DEBUG //To show during debug the messages that are being received
	pana_debug("Session updated with message:");
	if (type == PCI_MSG) { // PCI
        pana_debug("PCI");
    } else if (type == PAUTH_MSG) { //Authentication type Message
        if (flags & R_FLAG) { //PAR
            pana_debug("PAR");
        } else { // PAN
            pana_debug("PAN");
        }
    } else if (type == PTERM_MSG) {
        if (flags & R_FLAG) { //PTR
            pana_debug("PTR");
        } else { // PTA
            pana_debug("PTA");
        }
    } else if (type == PNOTIF_MSG) { //Notification Message
        if (flags & R_FLAG) { //PNR
            pana_debug("PNR");
        } else { // PNA
            pana_debug("PNA");
	...
	if (type == PAUTH_MSG) { //Authentication type Message
